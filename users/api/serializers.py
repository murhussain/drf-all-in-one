...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    def save(self):
        ...
        if User.objects.filter(username=self.validated_data['username']):
            raise serializers.ValidationError({'error': 'Your username has already been taken'})

        newAccount = User(email=self.validated_data['email'], username=self.validated_data['username'])
